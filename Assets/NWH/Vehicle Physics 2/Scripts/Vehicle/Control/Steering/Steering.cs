...
namespace NWH.VehiclePhysics2
{
    ...
    public class Steering : VehicleComponent
    {
        ...
        /// <summary>
        ///     "This is the maximum Torque that can be sent to a steering wheel in either direction. (Nm)")]
        /// </summary>
        [Tooltip(
            "This is the maximum Torque that can be sent to a steering wheel in either direction. (Nm)")]
        public float MaximumForceFeedbackSteeringTorque = 12f;
        ...
        public float CalcForceFeedbackSteeringTorque(VehicleController vc){

           /*  float SteeringTorque = Angle*(vc.Speed/50);
            

            if (vc.Speed > 30){
                SteeringTorque = Angle*(vc.Speed/100);
            } */

            // float torqueAngle = Angle;
            ...
            // float valueRateOfChange = (Angle - lastAngle) / Time.deltaTime;
            // lastAngle = Angle;

            // float D = 0.001f * -valueRateOfChange; //note the negative sign here
            ...
            float SteeringTorque = 2 * Mathf.Atan((Angle)*(vc.Speed/50));

            if (SteeringTorque > MaximumForceFeedbackSteeringTorque){
                SteeringTorque = MaximumForceFeedbackSteeringTorque;
            }
            if (SteeringTorque < -MaximumForceFeedbackSteeringTorque){
                SteeringTorque = -MaximumForceFeedbackSteeringTorque;
            }

            // float SteeringTorque = P+D;
